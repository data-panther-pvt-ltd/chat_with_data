...
export default function CSVAnalyzerDashboard() {
  ...
  const testServerConnection = async () => {
    ...
    try {
      ...
      const rootResponse = await fetch("http://34.70.203.66:8000/", {
        method: 'GET',
        headers: {
      ...
      const healthResponse = await fetch("http://34.70.203.66:8000/health");
  ...
  const loadCsvFiles = async () => {
    ...
    try {
      ...
      const requestUrl = "http://34.70.203.66:8000/list-csv";
  ...
  const deleteFile = async () => {
    ...
    try {
      const requestUrl = `http://34.70.203.66:8000/delete-csv?filename=${encodeURIComponent(filename)}`;
